...
const uploadImageToHtml = async (e) => {
  const email_content = e;
  const parser = new DOMParser();
  const doc = parser.parseFromString(email_content, "text/html");
  ...
  if (imgs.length > 0) {
    for (const i = 0; i < imgs.length; i++) {
      ...
      if (block.length > 1) {
        ...
        imgs[i].src = responImage.image_url;
      } //end if
    } //end for
  }
...
export default uploadImageToHtml